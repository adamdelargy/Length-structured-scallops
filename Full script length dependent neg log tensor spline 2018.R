...
# - tensor spline for fishing mortality 
# up to 2018 
...
#setwd("C:/Users/User/Documents/Bangor University/Stock assessment/length_assess/scallop length assess/Fished area/My modifications")

source("M:/My Documents/ICES advanced stock assessment/ICES_advanced_assessment_course/Exercises/Scripts/helper functions.R")
#source("C:/Users/User/Documents/Bangor University/ICES advanced stock assessment/ICES_advanced_assessment_course/Exercises/Scripts/helper functions.R")

#### data 
	
Ns = as.matrix(read.csv("Obs Nsurv all years.csv", header=F))
dimnames(Ns)   <-  list("length"=1:nrow(Ns)+1, "year" = 2012:2018) 
...
legend("topright", legend=2012:2018, lty=1,col=1:7)

# we can't have zeros in the model
# so we will start the model from 6cm 


Ns <- Ns[-c(1:4),]
# still a zero though in 6cm class in 2017 
# really don't want to lose this class as 56 were caught in 2018 
# plus there was also 0 8cm's caught in 2018
# nightmare 

# I'll add one in each to get model running 
...
Eff <- scan("Obs Effort full years.csv")
Eff[c(6:7)] <- NA
...
# The Fcalc is different in this assessment because we now use the tensor spline to generate the F at age matrix
Fcalc <- function(X,pars,mls){
	pars <- log(pars) 
  adj <- min(Lengths) - 1	# if classes don't begin at 1 
  admls <- mls - adj	# adjust mls as if it began at 1 
  res         <-  array(NA, dim=c(length(Lengths),length(years)))
  res[1:admls,]   <-  exp(matrix(X %*% pars, length(min(Lengths):mls)))
  res[(admls+1):length(Lengths),] <- rep(res[admls,], each= length(Lengths)-admls)
  return(res)
}
...
MakeParList <- function(fitting.pars,fixed.pars){
    attach(fixed.pars)
    n1<-length(fitting.pars)-maxiter-nhist-ncol(X)
    n2<-n1+ncol(X)
    n3<-n2+maxiter
    n4<-n3+nhist
    detach(fixed.pars)
...
len.sim<-function(par.list,N0=NULL) {
   ...
   F<-Fcalc(X,Ft,mls)					# here we apply selectivity to our fishing mortality estimates to get fishing mortality at size
   ...
   Ns<- if (Survey==T) survey(N,L,pA,alphaV,betaV,qmax) else empty	# here we use a logistic equation to estimate survey densities - this is the bit I could improve 
...
# instead of logistic curve we will be using tensor spline for fishing mortality
X  <- gam(dummy ~ te(length, year, k=c(3,3)), data = expand.grid(dummy = 1, length = Lmin:11, year = as.numeric(years)), fit = FALSE) $ X
...
Fscalar <- c(5.540443e-07,1.517928e+01,5.169612e+01,1.091895e-02,2.852741e-06,4.690611e+01,5.729021e+05,6.618023e+04,3.966102e+04)
#Fscalar <- rep(1,ncol(X))		# parameters describing Fa,t combined with the design matrix from GAM
...
#Rec <- rep(3e8,maxiter)
#RecHist <- rep(3e8, 5)
Rec <- c(4.395657e+09,1.165629e+10,2.925076e+06,1.565938e-05, 1.440732e+09, 1.315277e+09,3.000000e+08)
RecHist <- c(3.577716e+05,3.027302e+05,4.489693e+10,1.500067e+09,7.816668e+02)
pA<-c(8.409929e-06, 9.810846e-06, 9.836951e-06, NA, 1.920750e-05,7.949143e-06,4.551069e-06) 	# script in my modifications folder 
...
fitting.pars<-c(
	...
	betaR = 1.611683e+03,
	qmax = 3.986641e-01,
	sigma = c(7.117040e-05, 1.906425e+04,1.338573e-01, 1.099975e+00),		
    Ft		= scan("Ftsim.csv"),
    Rt		= scan("Rtsim.csv"),
...
plot(2012:2018,TSB,ylim=c(0,max(TSB)),xlab="year",ylab="TSB (1000's tonnes)",type="l")	
...
fun<-function(ln.fitting.pars,fixed.pars,obs){
	...
	sigmaU <- exp(log(pars[7])+log(pars[8])*Lengths+log(pars[9])*Lengths^2)


 	# calculate -log likelihood for two sources
  
  	nll_c <-  -sum(dnorm(log(mod$Lb),log(obs$Lb),pars[6],TRUE),na.rm=T)
...
eigen(fit2$hessian)$values

### CG fit 

fit3<-optim(ln.fitting.pars,fun,gr=NULL,fixed.pars,obs, method="CG",control=list(maxit=30000),hessian=T)            

pred3<-len.sim(MakeParList(exp(fit3$par),fixed.pars))

par(mfrow=c(3,3))
plot(obs$Lb,ylim=c(min(obs$Lb,pred0$Lb,pred3$Lb),max(obs$Lb,pred0$Lb,pred3$Lb)))
lines(pred0$Lb,lty=2)  
lines(pred3$Lb,col="red",lwd=2) 

for (i in 1:7){
plot(fixed.pars$L,obs$Ns[,i],ylim=c(min(obs$Ns,pred0$Ns,pred3$Ns, na.rm=T),max(obs$Ns,pred0$Ns,pred3$Ns, na.rm=T)))
lines(fixed.pars$L,pred0$Ns[,i],lty=2)         
lines(fixed.pars$L,pred3$Ns[,i],col="red",lwd=2)
abline(v=11,lty=2)
}

eigen(fit3$hessian)$values


### L-BFGS-B fit 

fit4<-optim(fitting.pars,fun,gr=NULL,fixed.pars,obs,
            method="L-BFGS-B",control=list(trace=T,maxit=30000),lower=rep(0,length(fitting.pars)),hessian=T)

pred4<-len.sim(MakeParList(fit4$par,fixed.pars))

par(mfrow=c(3,3))
plot(obs$Lb,ylim=c(min(obs$Lb,pred0$Lb,pred4$Lb),max(obs$Lb,pred0$Lb,pred4$Lb)))
lines(pred0$Lb,lty=2)  
lines(pred4$Lb,col="red",lwd=2) 

for (i in 1:7){
plot(fixed.pars$L,obs$Ns[,i],ylim=c(min(obs$Ns,pred0$Ns,pred4$Ns, na.rm=T),max(obs$Ns,pred0$Ns,pred4$Ns, na.rm=T)))
lines(fixed.pars$L,pred0$Ns[,i],lty=2)         
lines(fixed.pars$L,pred4$Ns[,i],col="red",lwd=2)
abline(v=11,lty=2)
}

eigen(fit4$hessian)$values

fit5<-optim(fitting.pars,fun,gr=NULL,fixed.pars,obs,
            method="SANN",control=list(trace=T,maxit=3000000),hessian=T)
...
par(mfrow=c(2,2))
plot(2012:2018,TSB,ylim=c(0,max(TSB)),xlab="year",ylab="TSB (Tonnes)",type="l")	
plot(2012:2018,Catch,ylim=c(0,max(Catch)),xlab="year",ylab="Catch (Tonnes)",type="l")	
plot(2012:2018,Abundance,ylim=c(0,max(Abundance)),xlab="year",ylab="Abundance (1000's of scallops)",type="l")	
plot(2012:2018,Fish.mort,ylim=c(0,max(Fish.mort)),xlab="year",ylab="Fishing mortality",type="l")	


par(mfrow=c(1,1))
plot(2012:2018,Catch,ylim=c(0,max(Catch,obs$Lb)),xlab="Year",ylab="Catch (Tonnes)",type="b",col=2, cex.lab=1.5, cex.axis=1.5,lwd=2)
lines(2012:2018, obs$Lb,type='l',lwd=2)
legend("topright", legend=c("Observed landings", "Modelled landings"), lty=1,col=1:2, cex=1.5)

plot(2012:2018,Abundance,ylim=c(0,max(Abundance)),xlab="Year",ylab="Abundance (1000's of scallops)",type="b", cex.axis=1.5,cex.lab=1.5,lwd=2)

par(mfrow=c(1,2))
plot(2012:2018,Fish.mort,ylim=c(0,max(Fish.mort)),xlab="Year",ylab="Fishing mortality",type="b",cex.axis=1.5,cex.lab=1.5,lwd=2)	
plot(2012:2018,Eff,ylim=c(0,max(Eff)),xlab="Year",ylab="Effective Effort (kwdays)",type="b",cex.axis=1.5,cex.lab=1.5,lwd=2)
...
par(mfrow=c(1,1))
plot(2012:2018,(Abundance*1000)/Sarea.dm,ylim=c(0,max((Abundance*1000)/Sarea.dm)),xlab="Year",ylab="Density (Count/100m2)",type="b",col=2, cex.axis=1.5,cex.lab=1.5,lwd=2)

survey.sa <- c(271.3372,316.5363,317.3785,NA,619.7090,256.4704,146.8353)
Nsum <- apply(obs$Ns,2,sum)
Ndens <- Nsum/survey.sa
lines(2012:2018,Ndens,type='b',lwd=2)
...
q <- exp(fit$par[1])*logistic(Lengths,exp(fit$par[2]),exp(fit$par[3]))
plot(q~Lengths)
...
# plot residuals from input data 

sdU <- exp(fit$par[5]+(fit$par[6]*Lengths)+(fit$par[7]*Lengths^2))
U.stdres <- log(pred$Ns/obs$Ns)
U.stdres <- apply(U.stdres,2,function(x,sdU){x <- x/sdU},sdU)
plot(Lengths,apply(U.stdres,1,sd,na.rm=T),type='l')
abline(h=1,col='magenta',lty=2)



#########################################################################
# estimate uncertainties by sampling from from vcov matrix
#########################################################################

eigen(fit$hessian)$values
eigen(fit2$hessian)$values

#estimate covariance matrix by inverse of Hessian
covMatrix<-solve(fit$hessian)
