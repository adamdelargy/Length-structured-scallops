...
Fscalar <- c(8.909686e-02,2.631551e-01,5.060673e-01,7.640335e+00,5.597631e-03,1.103096e-01,1.024437e-01, 3.334262e-01,1.048373e-01)
...
Rec <- c(1.415617e+08, 1.325619e+07,2.550469e+06,3.738278e+07,9.519651e-10)
RecHist <- c(5.892528e-01,  8.477007e+05,8.187937e-05,3.762383e+08,7.534628e+07)
...
fitting.pars<-c(
    	qmax	=  3.271740e-01,		# so qmax is the maximum theoretical absolute efficiency 
 	alphaV	= 2.577500e+01,		
    betaV	= 2.765021e+02,
	sigma = c(9.148018e-05,2.413701e+05,7.123502e-02, 1.133608e+00),		
    Ft		= scan("Ftsim.csv"),
    Rt		= scan("Rtsim.csv"),
...
fun<-function(ln.fitting.pars,fixed.pars,obs){
	...
	sigmaU <- exp(log(pars[5])+log(pars[6])*Lengths+log(pars[7])*Lengths^2)


 	# calculate -log likelihood for two sources
  
  	nll_c <-  -sum(dnorm(log(mod$Lb),log(obs$Lb),pars[4],TRUE),na.rm=T)
