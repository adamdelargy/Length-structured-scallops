...
fitting.pars<-c(
	...
	betaS =  4.717542e+00,
 	...
 	alphaV	= 5.686989e+00,		
    betaV	= 1.694036e-01,		# I obtained these parameters as guesses by adjusting a logistic curve until I found the shape I wanted
    Ft		= scan("Ftsim.csv"),
    Rt		= scan("Rtsim.csv"),
...
fun<-function(ln.fitting.pars,fixed.pars,obs){
	...
	return(sum((mod$Lb-obs$Lb)^2  + 1e4*(mod$Ns-obs$Ns)^2,na.rm=T))}
...
lines(2012:2016, obs$Lb,type='b',lwd=2)
