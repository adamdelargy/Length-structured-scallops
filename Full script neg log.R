...
### Full script 
# - negative loglikelihood, non-length dependent sigma 
# - logistic curve for fishing mortality 
...
Fscalar <- c( 4.176166e-01,1.257866e-01,2.389328e-01,3.861990e-01,4.616747e-02)
...
Rec <- c(9.408812e+07, 1.345922e+04,1.628108e+03,3.075828e+07,1.955964e-04)
RecHist <- c(4.452262e-02,1.111386e+03,5.820114e-01,2.633909e+08,3.111330e+06)
...
fitting.pars<-c(
	alphaS = 2.896325e+13,
	betaS =  4.717542e+00,
    	qmax	=  1.250340e+00,		# so qmax is the maximum theoretical absolute efficiency 
 	alphaV	= 5.686989e+00,		
    betaV	= 1.694036e-01,
	sigma = c(2.7,2.7),		# I obtained these parameters as guesses by adjusting a logistic curve until I found the shape I wanted
    Ft		= scan("Ftsim.csv"),
    Rt		= scan("Rtsim.csv"),
...
fun<-function(ln.fitting.pars,fixed.pars,obs){
	...
	mod<-len.sim(MakeParList(pars,fixed.pars))
	 # calculate -log likelihood for two sources
  	...
  	nll_s <-  -sum(dnorm(log(mod$Ns),log(obs$Ns),pars[7],TRUE),na.rm=T)
  	return(nll_c + nll_s  )
}
...
q <- exp(fit$par[3])*logistic(Lengths,exp(fit$par[4]),exp(fit$par[5]))
plot(q~Lengths)
